...
from image_processor import preprocess, standardize_serie, normalize_data
...
def extract_and_add_dcm_ext(dirclab_dir ='./', extract_the_zip = True, mask= 'liver', add_png= True, with_tumors = True):
    '''
    ...
    @param add_png "True" to add png images of each DICOM slice for the given mask
    ...
    for rep in os.listdir(dirclab_dir):
        if rep.startswith('3Dircadb1.') and os.path.isdir(dirclab_dir + rep):
            ...
            if add_png:
                from_dcm_to_png(full_patient_dir)
                if with_tumors:
                    iterate_in_masks(full_masks_dir, mask, from_dcm_to_png)
                else:
                    from_dcm_to_png(full_masks_dir + mask + '/')
